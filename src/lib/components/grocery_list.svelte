    ...
    const touch_move = (e) => {
        ...
        if (touchY < scrollRect.top + EDGE_THRESHOLD) {
            ...
            scrollInterval = setInterval(() => {
                const outside_item = document.getElementById(dragged_over).parentElement.previousElementSibling.previousElementSibling;
                if (outside_item){
                    outside_item.scrollIntoView({behavior: "smooth", block: "center"});
                    dragged_over = outside_item.getElementsByTagName("input")[0].id;
                    clearInterval(scrollInterval);
                    scrollInterval = null;
            ...
            isAutoScrolling = true;
            scrollInterval = setInterval(() => {
                const outside_item = document.getElementById(dragged_over).parentElement.nextElementSibling.nextElementSibling;
                if (outside_item) {
                    outside_item.scrollIntoView({behavior: "smooth", block: "center"});
                    dragged_over = outside_item.getElementsByTagName("input")[0].id;
                    clearInterval(scrollInterval);
        ...
        <div id="list" class="grocery_list h-[calc(100svh-155px)] md:h-[calc(100svh-105px)] overflow-y-auto px-2 py-4 border-t border-b border-base-300 md:border-none">
            {#if grocery_list.length > 0}
                {#each grocery_list as item, i}
                    ...
                    <div id={item.id} class="grocery_item select-none hidden md:flex space-x-3 justify-start items-center {item.id == dragged_item && item.id != dragged_over ? `border border-error rounded-lg p-1` : ``} {item.id == dragged_over ? `border border-primary rounded-lg p-1` : ``}"
                        draggable="true"
                    ...
                    >
                        ...
                        <div class="flex">
                            <div class="text flex flex-col space-x-2{item.id == dragged_item || item.id == dragged_over ? ` text-xl` : ``}">
                                <p>{ingrs_to_string([item])}</p>
                                <p class="text-neutral text-xs content-center pl-2">{get_recipe_name(item)}</p>
                            </div>
                        </div>
                        {#if status != "none"}
                    ...
                    <div class="grocery_item select-none flex md:hidden relative content-center overflow-hidden {item.id == dragged_item && item.id != dragged_over ? `border border-error rounded-lg` : ``} {item.id == dragged_over ? `border border-primary rounded-lg` : ``}">
                        ...
                        <div class="flex space-x-3 justify-end items-center bg-base-100 w-full transition-transform duration-200 ease-out"
                            ...
                            <div id={item.id} class="flex flex-col flex-1 py-1 flex-shrink"
                                ...
                                <p class="text-neutral text-xs text-center w-fit pl-2">{get_recipe_name(item)}</p>
                            </div>
                            {#if status != "none"}<input type="checkbox" class="md:hidden checkbox checkbox-primary checkbox-lg p-1" id={item.id} bind:checked={item.checked} onchange={check_item_handle}>{/if}
                        </div>
                    </div>
